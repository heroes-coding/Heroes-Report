...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({width: winState.width, height: winState.height, x: winState.x, y: winState.y, minWidth: 825, minHeight: 480, frame: false, standardWindow: false, show: false, webPreferences: {webSecurity: false}})
...
function showLoadingWindow() {
  loadingWindow = new BrowserWindow({width: 340, height: 225, resizable: true, frame: false, standardWindow: false, show: false})
  loadingWindow.once('ready-to-show', async() => {
    loadingWindow.show()
    loadPreviewWindow()
    createWindow()
  })
  const loadingURL = process.env.ELECTRON_START_URL ? path.join(process.env.ELECTRON_START_URL,'loading') : url.format({
