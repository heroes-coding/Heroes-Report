  ...
  render() {
    ...
    if (yMin === undefined) {
      yMin = bars
        ? 0
        : errorBars
        ? d3.min(errorBars.map(y => y[1]))
        : d3.min(ys);
    }
    if (yMax === undefined) {
      yMax = errorBars
        ? d3.max(errorBars.map(y => y[2]))
        : d3.max(ys.filter(y => y < Infinity));
    }
    ...
    const xScale = d3
      .scaleLinear()
      .range([toMap.length === 1 ? xRatio / 2 : xOff + 1, xRatio])
    ...
    const xTickRotation =
      xTicks[xTicks.length - 1].toString().length <= 2
        ? "rotate(-90)"
        : "rotate(-45)";
    const yScale = d3
      .scaleLinear()
      .range([yRatio - yOff - 1, 0])
    ...
    if (linePoints) {
      line = d3
        .line()
        .curve(step ? d3.curveStepBefore : d3.curveCatmullRom)
        .x(function(d) {
          return xScale(d[0]);
        })
        .y(function(d) {
    ...
    return (
      <div
        ...
        className={containerClass || "graphHolder"}
      >
        {labelPoints && (
          <Popup
        ...
        >
              ...
              return (
                <g key={y}>
                  <line
                    ...
                    className="tick"
                  />
                  <text x={xOff - 10} y={y + 5} className="tickText">
                    {tick}
                  </text>
                  <line
                    x1={xOff}
            ...
            bars.map((p, i) => {
              ...
              return (
                <g key={x}>
                  ...
                  />
                  <text className="barText" x={x} y={yMaxCoord + 25}>
                    {yVal}
                  </text>
                </g>
              );
          ...
          {lines &&
            lines.map((line, i) => (
              <path
                key={i}
                d={line}
                className="line multiline"
                style={{ stroke: colors[i] }}
              />
            ))}
          {lineLabels &&
            lineLabels.map((lineLabel, i) => {
          ...
          <text x={xOff + 10} y={yRatio - yOff - 10} className="axisText">
            {xLabel}
          </text>
          <text
            onMouseEnter={() => {
