...
class PlayerStatList extends Component {
  ...
  mapStat(stat, name) {
    ...
    const results = {
      ...
      right:
        stats.length > 1
          ? formatNumber(d3.deviation(stats)).toString()
          : "----",
      statName: name,
    };
    window.timings["Stat calculation (" + name + ")"] =
      Math.round(window.performance.now() * 100 - 100 * expTime) / 100;
    return results;
  }
  stat(statName, playerData, providedStats, shortName) {
    ...
    if (!providedStats) {
      ...
      stats = playerData.map(x =>
        x.stats[index] === Infinity ? 20 : x.stats[index]
      );
    } else {
      stats = providedStats;
    }
    ...
    const std = `${
      stats.length > 1 ? formatNumber(d3.deviation(stats)) : "---"
    }`;
    window.timings["Stat calculation (" + statName + ")"] =
      Math.round(window.performance.now() * 100 - 100 * expTime) / 100;
  ...
  percent(statName, playerData) {
    ...
    const stats = playerData
      .filter(
        x => (x[statName] === 1 && x.Won) || (x[statName] === 1 && !x.Won)
      )
      .map(x => x.Won);
    const negStats = playerData
      .filter(
        x => (x[statName] === 0 && x.Won) || (x[statName] === 0 && !x.Won)
      )
      .map(x => x.Won);
    /*
    if (stats.length === 0) {
    ...
    */
    ...
    const allyMean =
      stats.length > 0
        ? roundedPercent(Math.round(d3.mean(stats) * 1000)).toString()
        : "-----";
    const enemyMean =
      negStats.length > 0
        ? roundedPercent(Math.round(d3.mean(negStats) * 1000)).toString()
        : "-----";
    window.timings["Stat calculation (" + statName + ")"] =
      Math.round(window.performance.now() * 100 - 100 * expTime) / 100;
  ...
  mmr(mmrType, mmrData) {
    return {
      name: mmrType,
      left: mmrData.mmr.toString(),
      right: `(${formatPercentile(mmrData.percentile)})`,
    };
  }
  ...
  getGraphs() {
    ...
    if (stat.includes("First")) {
      let expTime = window.performance.now();
      const allyData = playerData
        .filter(x => (x[stat] === 1 && x.Won) || (x[stat] === 1 && !x.Won))
        .map(x => {
          return [x.MSL, x.Won];
        })
        .reverse();
      const enemyData = playerData
        .filter(x => (x[stat] === 0 && x.Won) || (x[stat] === 0 && !x.Won))
        .map(x => {
          return [x.MSL, x.Won];
        })
        .reverse();
      const timedData = [
        exponentialSmoothing(allyData),
        exponentialSmoothing(enemyData),
      ];
      window.timings["Winrate Correlation Data (" + statName + ")"] =
        Math.round(window.performance.now() * 100 - 100 * expTime) / 100;
    ...
    window.timings["Data Prep (" + statName + ")"] =
      Math.round(window.performance.now() * 100 - 100 * dataTime) / 100;
    ...
    if (showK) {
      if (isMapStat) {
        ...
        let expTime = window.performance.now();
        winrateCorrelationData = exponentialSmoothing(
          playerData
            .map((x, i) => {
              return [stats[i], x.Won];
            })
    ...
    if (isMapStat) {
      ...
      data = playerData
        .map((x, i) => {
          return [x.MSL, stats[i]];
        })
        .reverse();
    }
    yFormatter =
      statName.includes("Length") || statName.includes("Time")
        ? sToM
        : yFormatter;
    const timedData = exponentialSmoothing(data, 1, statName);
    window.timings["Winrate Over time Data"] =
      Math.round(window.performance.now() * 100 - 100 * expTime) / 100;
    ...
    return (
      <div className="graphs">
        ...
        />
        {showK && winrateCorrelationData.length && (
          <Graph
            graphClass="winrateGraph"
  ...
  render() {
    ...
    if (!this.props.isYou) {
      ...
      handle = this.props.playerInfo
        .filter(
          p => !yourSelectedAccount || yourSelectedAccount.handle === p.handle
        )
        .map(p => p.handle)
        .join(" | ");
      playerInfos = this.props.playerInfo;
    ...
    return (
      <div className="stat_item_container row">
        <StatListTemplate
          ...
          title={handle}
          subTitle={`Won ${roundedPercent(
            Math.round(d3.mean(outcomes) * 1000)
          )} (${d3.sum(outcomes)}/${outcomes.length}  matches)`}
          ...
          data={[
            {
              ...
              right: "Enemy",
              hasGraphs: true,
              stats: ["FirstTo10", "FirstTo20", "FirstFort"].map(s => {
                return percent(s, playerData);
              }),
            },
            {
              ...
              stats: [
                ...[
                  ...
                  ["Globes", null],
                  ["Experience", null],
                  ["Player Town Kills", "Town Kills"],
                  ["Team Town Kills", "Team T.Kills"],
                  ["Mercs Captured", "Mercs"],
                  ["Team Merc Captures", "Team Mercs"],
                  ["Time on Fire", null],
                  ["Escapes", null],
                ].map(s => {
                  return stat(s[0], playerData, null, s[1]);
                }),
              ],
            },
            {
              ...
              stats: [
                ...[
                  ...
                  .filter(s => mapSpecificStats[s].length)
                  .map(s =>
                    mapStat(mapSpecificStats[s], window.HOTS.nMapStat[s])
                  ),
              ],
            },
          ]}
          ...
          data={[
            {
              ...
              stats: [
                stat(
                  "KDA",
                  playerData,
                  playerData.map(x => (x.KDA === Infinity ? 20 : x.KDA))
                ),
                ...[
                  ["Kills", null],
                  ["Deaths", null],
                  ["Outnumbered Deaths", "Out#d Deaths"],
                  ["Assists", null],
                  ["Dead Time", "Dead Time"],
                  ["Vengeances", null],
                ].map(s => {
            ...
            },
            {
              ...
              stats: [
                ...[
                  ["Teamfight Damage Taken", "TF Dam.Rec."],
                  ["Healing", null],
                  ["Self Healing", "Self Healing"],
                  ["Protection Given", "Protection"],
                ].map(s => {
                  return stat(s[0], playerData, null, s[1]);
                }),
              ],
            },
            {
              ...
              stats: [
                ...[
                  ["Hero Damage", null],
                  ["Teamfight Hero Damage", "TF Hero Dam."],
                  ["Structure Damage", "Build. Dam."],
                  ["Minion Damage", "Minion Dam."],
                  ["Siege Damage", "Siege Dam."],
                ].map(s => {
                  return stat(s[0], playerData, null, s[1]);
                }),
              ],
...
function mapStateToProps(state, ownProps) {
  return {
    ...PlayerReplaysSelector(state),
    yourSelectedAccount: state.yourSelectedAccount,
  };
}
