...
let stat = (statName, playerData, providedStats, shortName) => {
  ...
  return (
    <div className="statBarHolder" key={statName}>
      ...
      <span className="statName">{statName}:</span>
      {getSpace(spaceLeft)}
      <span className="statValue">{mean}</span>
      {getSpace(spaceMiddle)}(<span className="stdValue">{std}</span>)
    </div>
  );
};
...
let percent = (statName, playerData) => {
  ...
  const stats = playerData
    .filter(x => (x[statName] === 1 && x.Won) || (x[statName] === 1 && !x.Won))
    .map(x => x.Won);
  const negStats = playerData
    .filter(x => (x[statName] === 0 && x.Won) || (x[statName] === 0 && !x.Won))
    .map(x => x.Won);
  if (stats.length === 0) {
    return <div />;
  }
  ...
  const allyMean =
    stats.length > 0
      ? roundedPercent(Math.round(d3.mean(stats) * 1000))
      : "-----";
  const enemyMean =
    negStats.length > 0
      ? roundedPercent(Math.round(d3.mean(negStats) * 1000))
      : "-----";
  const spaceLeft =
    spacesLeft - statName.length - allyMean.toString().length + 2;
  ...
  return (
    <div className="statBarHolder" key={statName}>
      ...
      <span className="statName">{statName}:</span>
      {getSpace(spaceLeft)}
      <span className="statValue">{allyMean}</span>
      {getSpace(spaceMiddle)}
      <span className="stdValue">{enemyMean}</span>
    </div>
  );
};
...
let mmr = (mmrType, mmrData) => {
  ...
  return (
    <div className="statBarHolder" key={mmrType}>
      <span className="statName">{mmrType}:</span>
      {getSpace(spaceLeft)}
      <span className="statValue">{mmrData.mmr}</span>
      {getSpace(spaceMiddle)}
      <span className="stdValue">{formatPercentile(mmrData.percentile)}</span>
    </div>
  );
};
...
class StatList extends Component {
  ...
  render() {
    ...
    console.log(
      `It took ${Math.round(window.performance.now() * 100 - 100 * expTime) /
        100} ms to calculate exponentially smoothed line`
    );
      ...
      <div className="stat_item_container row">
        <div className="statItem col-12 col-sm-6 col-lg-12">
          <div className="handleHolder statBarHolder statBarTitle">
            ...
            <br />
            <span id="winrate">
              Won {roundedPercent(Math.round(d3.mean(outcomes) * 1000))} (
          ...
          <div className="statBarHolder statBarTitle">
            {getSpace(3)}Who Got It ----- {getSpace(spacesLeft - 18)}Ally
            {getSpace(spacesMiddle - 5)}Enemy
            <br />
            {getSpace(3)}
            <span className="underline">What Event</span>
            {getSpace(spacesLeft + spacesMiddle - 19)}(
            <span className="underline">Your Win %</span>)
          </div>
          {percent("FirstTo10", this.props.playerData)}
          {percent("FirstTo20", this.props.playerData)}
          {percent("FirstFort", this.props.playerData)}
          <div className="statBarHolder statBarTitle">
            {getSpace(3)}Stat{getSpace(spacesLeft - 7)}Mean
            {getSpace(spacesMiddle - 3)}Sigma
          </div>
          {stat(
            "KDA",
            this.props.playerData,
            this.props.playerData.map(x => (x.KDA === Infinity ? 20 : x.KDA))
          )}
          ...
          <div className="statBarHolder statBarTitle">
            MMR Type{getSpace(spacesLeft - 11)}MMR{getSpace(spacesMiddle - 2)}
            Perc.
          </div>
          {h && mmr(mmrNames.h, h)}
          {q && mmr(mmrNames.q, q)}
