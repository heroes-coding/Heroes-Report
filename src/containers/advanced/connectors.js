...
const fetchAndInstantiate = (url, importObject) => {
  return window
    ...
    .then(bytes => window.WebAssembly.instantiate(bytes, importObject))
  ...
  for (let a = 0; a < nAllies; a++) {
    ...
    for (let l = 0; l < 7; l++) {
      ...
      for (let t = 0; t < nTals; t++) {
        ...
        allyTalSpecifics = allyTalSpecifics.concat([
          slot,
          builds.length,
          ...builds,
        ]);
...
const loadRustyReplays = () => {
  let promise = new Promise(function(resolve, reject) {
    ...
    window.rustyReplays.addBasics = HOTS => {
      ...
      const rolesAndFranchises = Object.values(HOTS.franchiseN).concat(
        Object.values(HOTS.roleN)
      );
    ...
    window.rustyReplays.addReplays = (
      replayInts,
      daysSinceLaunch,
      mode,
      density
      ...
      let totalReplays = window.rustyReplays.add_replays(
        ptr,
        nReplays,
        daysSinceLaunch,
        mode,
    ...
    window.rustyReplays.filterData = props => {
      ...
      filterHeroes[0]
        .filter(x => isNaN(x))
        .map(x => {
          allyRoles[roleDic[x.id]] = x.count;
        });
      Object.keys(allyRoles).map(k => {
        const parts = allyRoles[k].split(" ");
        const operator =
          parts.length > 1 ? ["", "<", ">"].indexOf(parts[0]) : 0;
      ...
      filterHeroes[1]
        .filter(x => isNaN(x))
        .map(x => {
          enemyRoles[roleDic[x.id]] = x.count;
        });
      ...
      let allyTalents = [];
      const allyTalentData = filterTalentHeroes[0]
        .filter(x => !x.id)
        .map(x => x.talents);
      let enemyTalents = [];
      const enemyTalentData = filterTalentHeroes[1]
        .filter(x => !x.id)
        .map(x => x.talents);
      allyTalents = loadTalentData(allyTalents, allyTalentData, allies.length);
      enemyTalents = loadTalentData(
        enemyTalents,
        enemyTalentData,
        enemies.length
      );
      ...
      if (!nFiltered)
        return {
          stats: [],
          winrateData: { errorBars: [], labelPoints: [], data: [] },
        };
      return window.rustyReplays.getStats(
        allies,
        allies.length,
        minMSL,
        maxMSL,
    ...
    window.rustyReplays.getFilteredMSL = () => {
      ...
      let nFiltered = new Uint32Array(
        window.rustyReplays.memory.buffer.slice(ptr, ptr + 4)
      )[0];
      ptr = ptr + 4;
      return Array.from(
        new Uint32Array(
          window.rustyReplays.memory.buffer.slice(ptr, ptr + nFiltered * 4)
        )
      );
    };
    window.rustyReplays.getStats = (
      allies,
      nAllies,
      minMSL,
      maxMSL,
      ...
      let floatPtr = window.rustyReplays.get_stats(
        aTeamPtr,
        alliesUsed,
        density
      );
      const nBuilds = new Float32Array(
        window.rustyReplays.memory.buffer.slice(floatPtr, floatPtr + 4)
      )[0];
      floatPtr += 4;
      const buildData = Array.from(
        new Float32Array(
          window.rustyReplays.memory.buffer.slice(
            floatPtr,
            floatPtr + nBuilds * 4 * 3
          )
        )
      );
      const builds = Array(nBuilds)
        .fill()
        .map((_, i) => {
          return [buildData[i * 3], buildData[i * 3 + 1], buildData[i * 3 + 2]];
        });
      floatPtr += nBuilds * 4 * 3;
      const nPoints = new Float32Array(
        window.rustyReplays.memory.buffer.slice(floatPtr, floatPtr + 4)
      )[0];
      floatPtr += 4;
      const expArray = Array.from(
        new Float32Array(
          window.rustyReplays.memory.buffer.slice(
            floatPtr,
            floatPtr + nPoints * 4
      ...
      floatPtr += nPoints * 4;
      const data = Array(nPoints / 2)
        .fill()
        .map((_, i) => {
          return [expArray[i], expArray[i + nPoints / 2]];
        });
      let xArray, minDates;
      if (density === 666) {
        minDates = [];
        ...
        xArray = data.map((_, i) => Math.max(minMSL, maxMSL - i * density));
      // data based on minutes-density
      else {
        // monthly data
        xArray = [];
        minDates = [];
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          if (i > 0) {
            xArray.push(DateToMSL(upperMonth));
          }
          if (i < data.length - 1) {
            let month = (m - i) % 12;
      ...
      const meanTotal =
        data.map(y => y[1]).reduce((a, b) => a + b) / data.length;
      const labelPoints = data
        .map((y, i) => {
          const [wins, total] = y;
          const val = roundedPercent((wins / total) * 1000);
          const MSL = xArray[i];
          let desc = `The allied team won ${val} of ${total /
            5} matches (95% win rate confidence interval for all matches: ${roundedPercent(
            errorBars[i][1] * 1000
          )}-${roundedPercent(errorBars[i][2] * 1000)}) from ${MSLToDateString(
            minDates ? minDates[i] : Math.max(MSL - density, minMSL)
          )} to ${MSLToDateString(MSL)}.`;
          const buildInfo =
            window.builds[
              window.buildDic[builds[i] ? parseInt(builds[i][0]) : 999]
            ];
          if (!buildInfo && density === 666) return null;
          if (density === 666 && buildInfo)
            desc = `${desc} (during build ${buildInfo.name})`;
          return {
      ...
      const lineData = data.map((y, i) => {
        return [xArray[i], y[0] / y[1]];
      });

      let offset = (window.HOTS.fullHeroNames.length + 1) * nStats * 4;
      const stats = Array.from(
        new Float32Array(
          window.rustyReplays.memory.buffer.slice(floatPtr, floatPtr + offset)
        )
      );
      // THIS CAUSES ERRORS, NOT SURE WHY BUT LEAVING IT CAUSES A MEMORY LEAK: window.rustyReplays.dealloc(floatPtr, offset)
      return {
        stats: convertRustyStats(stats),
        winrateData: { errorBars, labelPoints, data: lineData },
      };
    };
    ...
    fetchAndInstantiate("/rustyReplays.wasm", imports).then(mod => {
