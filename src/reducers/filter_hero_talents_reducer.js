...
export default function(state = [[],[],[]], action) {
  ...
  if (action.type === ADD_HERO_FILTER) {
    ...
    if (action.team===4) { // switch teams
      ...
      // not applicable
      const sameRole = newState[action.team].map((x,i) => [x,i]).filter(x => typeof x[0] === 'object' && !x[0].hasOwnProperty('hero') && x[0].id.includes(action.hero.id))
      if (sameRole.length) newState[action.team][sameRole[0][1]].count = action.hero.count
      else newState[action.team].push(action.hero)
    } else {
      newState[action.team].push({hero:action.hero,talents:[{},{},{},{},{},{},{}] })
    }
    return newState
  }
  return state
