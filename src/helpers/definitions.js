...
export const statCatChoices = statCats.map((x, i) => {
  return { name: x, id: x };
});
...
for (let m = 0; m < mmrData.names.length; m++) {
  mmrChoices.push({
    stars: mmrData.stars[m],
    name: mmrData.names[m],
    id: mmrData.ids[m],
  });
}
