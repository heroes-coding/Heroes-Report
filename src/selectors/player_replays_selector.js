...
const getPlayerBaseData = (
  ...
  const allFranchises = Object.keys(franchises)
    .map(x => franchises[x].selected)
    .filter(x => x).length
    ? false
    : true;
  const allRoles = Object.keys(roles)
    .map(x => roles[x].selected)
    .filter(x => x).length
    ? false
    : true;
  if (!roleDic) {
  ...
  for (let r = 0; r < nReplays; r++) {
    ...
    if (selectedCoplayer) {
      ...
      if (rep.bnetIDs.includes(selectedCoplayer.bnetID)) {
        ...
        rep.coplayerIsAlly =
          Math.floor(rep.bnetIDs.indexOf(rep.bnetID) / 5) ===
          Math.floor(rep.coplayer / 5);
        filteredReplays.push(rep);
      }
    } else if (
      (timeRange && (MSL < timeRange[0] || MSL > timeRange[1])) ||
      (!isNaN(playerHero) && rep.hero !== playerHero) ||
      ...
      missingHeroes(rep) ||
      (heroTerms && !heroTerms.includes(rep.hero)) ||
  ...
  for (let n = 0; n < Math.ceil(nFiltered / replaysPerPage); n++) {
    pageNames.push({
      name: `Replays ${1 + n * replaysPerPage} - ${Math.min(
        (n + 1) * replaysPerPage,
        nFiltered
      )}`,
      id: n,
  ...
  if (window.HOTS && window.HOTS.nMapStat) {
    Object.keys(window.HOTS.nMapStat).map(k => {
      mapSpecificStats[k] = [];
    });
    for (let r = 0; r < nFiltered; r++) {
      ...
      if (ID0) {
        mapSpecificStats[ID0 - 1].push([
          MSL,
          stats[mapStatValues0] * statMults[ID0 - 1],
          Won,
        ]);
      }
      let ID1 = stats[mapStatID1];
      if (ID1) {
        mapSpecificStats[ID1 - 1].push([
          MSL,
          stats[mapStatValues1] * statMults[ID1 - 1],
          Won,
        ]);
      }
    }
