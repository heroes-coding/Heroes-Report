...
const MMRDropdowns = [
  ...
  { name: "Storm / Hero League", id: "h" },
  { name: "Team League", id: "t" },
  { name: "Unranked Draft", id: "u" },
];
const MMRNames = {};
MMRDropdowns.map(m => {
  MMRNames[m.id] = m.name;
});
      ...
      <div className="overall optionsDiv">
        <div className="electronHeader">
          <TrafficLights
            ...
            noMin={true}
          />
          <div className="optionsTitle">Match Preview</div>
        </div>
        <div className="ReactTable -striped -highlight accountList">
          <div className="rt-table">
            <div className="rt-thead -header">
              ...
              <div className="rt-tr">
                <div className="rt-th replayFileList -cursor-pointer">
                  Player
                </div>
            ...
            <div className="rt-tbody">
              {this.state.players.map((p, i) => {
                ...
                const playerID =
                  i === you ? "you" : mmrs ? `${mmrs.region}-${mmrs.id}` : null;
                let winPercentWith =
                  nWith && nWith !== "-"
                    ? simplePercent(nWinWith / nWith)
                    : "-";
                let winPercentVS =
                  nVS && nVS !== "-" ? simplePercent(nWinVS / nVS) : "-";
                const MMRInfo = mmrs
                  ? mmrs[mode]
                    ? mmrs[mode]
                    : mmrs["q"]
                ...
                const percentile = MMRInfo
                  ? formatPercentile(MMRInfo.percentile)
                  : "-";
                ...
                const rowClass =
                  i === you
                    ...
                    : "";
                const label =
                  i === you ? "you" : isAlly ? "ally" : isEnemy ? "enemy" : "";
                const teamClass = p.team + (p.team ? Math.floor(i / 5) * 2 : 0);
                let partyStyle = {
                  backgroundColor: teamClass ? colors10(teamClass) : "none",
                };
                return (
                  <div
                    key={i}
                  ...
                  >
                    <div className={`rt-tr replayItem addPath`}>
                      <div
                        key={teamClass}
                        className="teamPartyBar"
                        style={partyStyle}
                      />
                      <div className={`rt-td handleCell  ${rowClass}`}>
                        {fullHandle} ({label})
                        <div className="unavailable">
                          {playerID ? "" : "PLAYER PROFILE UNAVAILABLE"}
                        </div>
                      </div>
